...
<div id="toolbar" class="btn-group pull-right" style="margin-right: 20px;">
    <button id="btn_add" type="button" class="btn btn-primary" data-toggle="modal" data-target="#addDailyLearning">
        <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增
    </button>
</div>

<div class="modal fade" id="addDailyLearning" tabindex="-1" role="dialog" aria-labelledby="addDailyLearningLabel" aria-hidden="true">
    <form role="form" id="addDailyLearningForm" method="post">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title" id="addDailyLearningLabel">添加日常学习</h4>
                </div>
                ...
                <div class="modal-footer" style="padding-top: 50px;">
                    ...
                    <button id="addDailyLearningBtn" type="button" class="btn btn-primary">添加</button>
                </div>
            </div>
        </div>
    </form>
...
<script>
    ...
    $('#addDailyLearning').on('show.bs.modal', function () {
        $(':input','#addDailyLearningForm')
            .not(':button, :submit, :reset, :file')
            .val('');
    });
    $("#addDailyLearningBtn").click(function () {
        $("#addDailyLearningForm").ajaxSubmit({
            type:'post',
            url:'${basePath}dailyLearning/manager/addDailyLearning',
            dataType: 'json',
            success:function(data){
                console.log(data);
                if(data.success){
                    $("#addDailyLearning").modal("hide");
                    refresh();
                }else{
                    alert("添加失败");
                }
    ...
    $('#mytab').bootstrapTable({
        ...
        url:"${basePath}dailyLearning/manager/dailyLearningList",//要请求数据的文件路径
    ...
    function del(__id) {
        ...
        $.confirm({
            title: '警告',
            content: '您确认要删除该日常学习记录吗？',
            buttons: {
                删除: function(){
                    $.ajax({
                        type: "POST",
                        url:'${basePath}dailyLearning/manager/delDailyLearning',
                        async: true,
                        data: {
                            id: __id
                        },
    ...
    function refresh() {
        $('#mytab').bootstrapTable('refresh', {url: '${basePath}dailyLearning/manager/dailyLearningList'});
    }
