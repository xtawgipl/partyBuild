...
<body>
    ...
    <div id="toolbar" class="btn-group pull-right" style="margin-right: 20px;">
        <button id="btn_add" type="button" class="btn btn-primary" data-toggle="modal" data-target="#addPartyFee">
            <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增
        </button>
    </div>

    <div class="modal fade" id="addPartyFee" tabindex="-1" role="dialog" aria-labelledby="addPartyFeeLabel" aria-hidden="true">
        <form role="form" id="addPartyFeerForm" method="post" enctype="multipart/form-data">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title" id="addPartyFeeLabel">添加党费缴纳情况</h4>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="form-group" style="padding-top: 30px;">
                            <label class="col-sm-2 form-label" for="dateTime">党费月份</label>
                            <div class="col-sm-4">
                                <input type="text" name="dateTime" class="datepicker form-control" id="dateTime" placeholder="请选择月份" />
                            </div>
                        ...
                        <div class="form-group" style="padding-top: 30px;">
                            <label class="col-sm-2 form-label" for="desc">描述</label>
                            <div class="col-sm-10">
                                <textarea rows="3" name="desc" class="form-control" id="desc" placeholder="党费缴纳描述"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer" style="padding-top: 50px;">
                        <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                        <button id="addPartyFeeBtn" type="button" class="btn btn-primary">添加</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    <div id="treePanle" style="display: none; background-color: #FFFFFF; z-index: 1051;">
        ...
        <div style="width: 100%;overflow:auto; height: 300px;" id="orgTree">
...
<script>
    ...
    $('#addPartyFee').on('show.bs.modal', function () {
        $(':input','#addPartyFeerForm')
            .not(':button, :submit, :reset, :file')
            .val('');
    });
    $("#addPartyFeeBtn").click(function () {
        $("#addPartyFeerForm").ajaxSubmit({
            type:'post',
            url:'${basePath}partyFee/manager/addPartyFee',
            dataType: 'json',
            success:function(data){
                console.log(data);
                if(data.success){
                    $("#addPartyFee").modal("hide");
                    refresh();
                }else{
                    alert("添加失败");
                }
    ...
    $('#mytab').bootstrapTable({
        ...
        url:"${basePath}partyFee/manager/partyFeeList",//要请求数据的文件路径
    ...
    function del(__id) {
        ...
        $.confirm({
            title: '警告',
            content: '您确认要删除该党费缴纳记录吗？',
            buttons: {
                删除: function(){
                    $.ajax({
                        type: "POST",
                        url:'${basePath}partyFee/manager/delPartyFee',
                        async: true,
                        data: {
                            id: __id
                        },
    ...
    function queryParams(params){
        ...
        return{
            ...
            "policemanBean.name":$('#search_name').val()
        }
    }
    //查询按钮事件
    ...
    function refresh() {
        $('#mytab').bootstrapTable('refresh', {url: '${basePath}partyFee/manager/partyFeeList'});
    }
